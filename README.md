...
<details>
   <summary> 2. 커뮤니티 무한스크롤 기능 </summary>
   
- 기존의 페이징은 offset과 limit을 사용해서 페이징할 범위를 정하지만, 이 방식은 초반에는 효율이 나쁘지 않지만 뒤로 갈수록 효율이 급격히 떨어진다는 단점이 있어 사용하지 않고 무한스크롤로 구현하고자 함.
- useInview 라이브러리를 사용해 스크롤이 맨밑에 닿을 시 페이지 번호가 증가하도록 구현함.
- 페이지 번호를 back으로 전달해 spring Boot의 JPA pageable 기능을 사용해 스크롤 시 페이징 되도록 구현함.
</details>
