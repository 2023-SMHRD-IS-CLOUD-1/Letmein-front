...
const Mypage = () => {
  ...
  const myPost = () => {
    setMyWrite(true);
    setMyLikeList(false);
    axios.get(`/searchWriter?page=${page}&size=6&userId=${user_id}`
       // 현재 페이지 번호를 백엔드로 전달
    )
    .then((res) => {
  ...
  const myLike = () => {
    setMyWrite(false);
    setMyLikeList(true);
    axios.get(`/myLike?user_id=${user_id}`
    ).then((res)=>{
...
const goToTop = () => {
  ...
  return (
    <div className='mypage-container'>
      ...
      <p style={{fontSize:'40px'}}>{user_nick}</p>
      <p style={{fontSize:'30px'}}>{user_id}</p>
