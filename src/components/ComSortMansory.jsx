...
const ComSortMansory = ({ searchKey, search, sort }) => {
  ...
  const [chkSort, setChkSort] = useState(false);
    ...
    // 제목기준 검색
    const searchTitle = () =>{
        axios.post(`http://54.180.13.94:8090/letmein/sortTitle`,{
      ...
      // 작성자 기준 검색
      const searchWriter = () => {
        axios.post(`http://54.180.13.94:8090/letmein/sortWriter`,{
            user_id : searchKey
        })
        .then((res)=>{
          if(res.data.length == 0){
        ...
        const sortLike = () => {
            if(del){
                axios.get("http://54.180.13.94:8090/letmein/sortLike")
                    .then((res) => {
                        setList("");
                        setHow(false);
